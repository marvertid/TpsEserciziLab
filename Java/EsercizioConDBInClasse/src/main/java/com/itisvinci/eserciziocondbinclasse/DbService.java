...
import com.itisvinci.eserciziocondbinclasse.data.EsempioData;
import com.itisvinci.eserciziocondbinclasse.data.SampleDbData;
...
public class DbService extends BaseDbService {
    ...
    public static List<EsempioData> selectEsempio(String surname) {
        try {
            List<EsempioData> result = new ArrayList<>();
            Connection con = getConnection();
            PreparedStatement stmt = con.prepareStatement(
                    "select colonna1" +
            ...
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                result.add(new EsempioData(rs.getString("colonna1")));
            }
            rs.close();
            stmt.close();
            con.close();
